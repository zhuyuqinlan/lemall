...
import org.zhuyuqinlan.lemall.common.entity.PmsSkuStock;
...
public class OmsPromotionService {
    private final PortalProductDao portalProductDao;

    public List<CartPromotionItem> calcCartPromotion(List<OmsCartItemRequestDTO> cartItemList) {
        List<OmsCartItem> omsCartItems = cartItemList.stream().map(e -> {
            OmsCartItem omsCartItem = new OmsCartItem();
            BeanUtils.copyProperties(e, omsCartItem);
            return omsCartItem;
        }).toList();
        //1.先根据productId对CartItem进行分组，以spu为单位进行计算优惠
        Map<Long, List<OmsCartItem>> productCartMap = groupCartItemBySpu(omsCartItems);
        //2.查询所有商品的优惠相关信息
        List<PromotionProduct> promotionProductList = getPromotionProductList(omsCartItems);
        ...
        for (Map.Entry<Long, List<OmsCartItem>> entry : productCartMap.entrySet()) {
            ...
            if (promotionType == 1) {
                ...
                PmsProductLadder ladder = getProductLadder(count, promotionProduct.getProductLadderList().stream().map(e -> {
                    PmsProductLadder pmsProductLadder = new PmsProductLadder();
                    BeanUtils.copyProperties(e,pmsProductLadder);
                ...
                PmsProductFullReduction fullReduction = getProductFullReduction(totalAmount,promotionProduct.getProductFullReductionList().stream().map(e -> {
    ...
    private PmsSkuStock getOriginalPrice(PromotionProduct promotionProduct, Long productSkuId) {
        for (PmsSkuStock skuStock : promotionProduct.getSkuStockList().stream().map(e -> {
